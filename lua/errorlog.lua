...
--    filename = string.format(filename, os.date(datePattern))
    ...
    return logging.new( function(self, level, message)
                            local fn = string.format(filename, os.date(datePattern))
                            local f = io.open(fn, "a")
                            if not f then
                                return nil, string.format("file `%s' could not be opened for writing", fn)
                            end
...
logger = logging.file("/var/log/lua"..(masterid or "")..".log")
