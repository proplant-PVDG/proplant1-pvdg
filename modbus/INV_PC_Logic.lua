...
function logCsvDg(file, msg)
 ...
 if file8 == nil then
  ...
  file8:write("TS,CASE,DG01_PAC,DG02_PAC,TOTAL_DG_PAC,TOTAL_DG_ONLINE,PV_PAC,PAC_LIMIT,GRID_CONNECT,PAC_LIMIT_WRITE".."\n")
 end
 file8:close()
  ...
  if ((dg01Pac < 1) and (dg02Pac < 1))  then
   ...
   if ((gridConn == 0) or (pacLimit ~= 100)) then
    ...
    if (gridConn == 0) then
     ...
     WR.writeHexOpts(dev, pacLimitWrite, bit.tohex(pacLimit,0),0x6)
  ...
  elseif (totaldgPac >= 0.1) then
   ...
   elseif (totaldgPac <= dg01ForceTuneDownX) then
    if (pacLimit >= 0) then
     ...
     WR.writeHexOpts(dev, pacLimitWrite, bit.tohex(pacLimit,4),0x6)
