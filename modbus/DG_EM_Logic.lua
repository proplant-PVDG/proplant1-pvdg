...
local dg1_pac = WR.read("SN:DG04_EM", "DG01_PAC")
local dg2_pac = WR.read("SN:DG04_EM", "DG02_PAC")
local dg3_pac = WR.read("SN:DG03_EM", "PAC")
local dg4_pac = WR.read("SN:DG04_EM", "PAC")
local totaldgpac = (dg1_pac + dg2_pac +dg3_pac + dg4_pac)
...
if not(settings) then
 ...
 CHECKDATATIME(dev, now, "DG_START_TIME")
 CHECKDATATIME(dev, now, "DG_STOP_TIME")
...
startTime[dev] = startTime[dev] or {ts=WR.read(dev, "DG_START_TIME")}
if is_nan(startTime[dev].ts) then startTime[dev].ts = 0 end
stopTime = stopTime or {}
stopTime[dev] = stopTime[dev] or {ts=WR.read(dev, "DG_STOP_TIME"), againStart=0}
...
opTime[dev].ts = now
WR.setProp(dev, "DG_START_TIME", startTime[dev].ts)
WR.setProp(dev, "DG_STOP_TIME", stopTime[dev].ts)
